...
-- Optimized for Supabase with proper table creation order
...
-- Conferences Table (no dependencies)
...
-- Seasons Table (self-referencing, create early)
...
-- Teams Table (depends on conferences)
...
-- Players Table (depends on teams and users)
...
-- ELO Players Table (no dependencies)
...
-- ELO Lobbies Table (depends on elo_players)
...
-- ELO Matches Table (depends on elo_lobbies)
...
-- Forum Categories Table (no dependencies)
...
-- Forum Posts Table (depends on forum_categories and auth.users)
...
-- Now create all remaining tables that depend on the above tables
